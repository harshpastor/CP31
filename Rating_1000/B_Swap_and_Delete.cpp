...
#define fr(i,j) foreach(i,0,j,1)
#define frr(i,j) rforeach(i,j,0,1)
...
void getv(vi &vec ){for(int i=0;i<vec.size();i++)cin>>vec[i]; }
void getv(vi &vec,int n ){for(int i=0;i<vec.size();i++)cin>>vec[i]; }
...
void solve()
{
    ...
    string s;
    cin>>s;
    //if zeros == ones means cost is zero.. thats the only condtn
    int zeros =  0 ;
